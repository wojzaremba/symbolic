...
\framebox[3.25in][c]{\parbox{3.0in}{{\bf Toy example:}
...
\vspace{1mm} \\ 
Naively this takes $O(nmk)$ time. However, our framework automatically
derives the following formula, that computes the same result in $O(n(k+m))$
time:
 \vspace{1mm} \\
%\begin{verbatim}
\texttt{	sum(sum(A .* repmat(sum(B, 2), [1, n])'))}
 \vspace{1mm} \\ This may be verified emprically:
