...
{\bf Computation Time} - It is a computation time for a particular architecture and programming language, or it can represent computation complexity. For a computation $\mathcal{C}$, we refer to this time as $t_{\mathcal{C}}$
...
greedy algorithm. Algorithm \ref{pseudocode} presents pseudo-code.

\begin{algorithm}
\caption{Find computation for expression}
\begin{algorithmic} 
\REQUIRE Target expression $T$, initial expression $W$, Rules $R_1, \dots, R_n$, maximum degree of polynomials $k$
\ENSURE Computation $\mathcal{C}$ of expression $T$.
