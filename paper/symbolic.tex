...
\STATE Get any literal $L$ from the $\mathbb{S}$
\STATE Get any rule $R$
\STATE Apply rule $R$ to $L$ to get $L'$
\STATE Continue if degree of $L'$ is not larger than $k$.
\IF {$L'.expression \in \mathbb{S}$}
	\IF {$L'.computation_time$ faster than for previous expression in $\mathbb{S}$} 
...
Expressions can be represented in various way. We present three approaches, which we explored.
...
recovered based on such values. However, it turns out, that this methods is numerically unstable
...
correspond just to the same operations in our representation. i.e. For element-wise multiplication of matrices of expressions, we
...
it might be quite slow. For instance, if two polynomials, which we want to multiply consist of 1000 coefficients, then
it takes $\sim 1mln.$ operations. We have implemented expressions in such representation, and it allowed us
to find patterns for polynomials up to degree $4$ (above $4$ it was consuming too much time).
...
to avoid numerical errors, we use integers, and we perform computation in the group $\mathbb{Z}_p$ (for large prime number $p$). This
guarantees that all the results of the computation can be represented correctly. Nonetheless, there are few potential
